angular
    ...
    .controller('dashboardCtrl', [
        ...
        '$localStorage',
		'$filter',
        function ($rootScope,$scope,$interval,$window,$timeout,variables, $http, $localStorage,$filter) {
        // circular statistics
        ...
        // calendar
			...
			// Last 5 days attendance percentages
			
			$scope.newData=['Student',];
			$scope.newDate=['x',];
			$scope.newData1=[];
			$http({
			method:'get',
			url: $localStorage.service+'DashboardAPI/last5daysRegCalc',
			headers:{'access_token':$localStorage.access_token}
			}).then(function(return_data){
            ...
            // $scope.video_data = [
                // {
                    ...
                    // UPLOAD_FILE: 'default/noData.jpg'
                // }
            // ];
            ...
            // }).then(function(return_data){
				// if(return_data.data.status==false){
					// $scope.videoPath="default/noData.jpg";
					// $scope.videoChange("start",$scope.videoPath);
					// $scope.posts_data=$scope.video_data;
					// $scope.postLength=false;
				// }else{
					// $scope.posts_data=return_data.data.message;
					// console.log($scope.posts_data,"$scope.posts_data");
					// $scope.postLength=true;
					// $scope.videoPath=$localStorage.uploadUrl+$scope.posts_data[0].UPLOAD_FILE;
					// $scope.videoChange("start",$scope.posts_data[0].UPLOAD_FILE);
				// }
