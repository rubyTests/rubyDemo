angular
    ...
    .controller('studentleaveapproveCtrl', [
		...
		'$http',
		'$localStorage',
        function ($scope,$rootScope,$timeout,$compile,variables,ts_data,$resource,$filter,$http,$localStorage) {
			...
			$scope.getStuData=function(){
				...
				if($scope.stuAttendanceReport.course==undefined || $scope.stuAttendanceReport.course==''){
					...
					$scope.table_data = [];
					$localStorage.stuLeaveCourse=$scope.stuAttendanceReport.course;
					$localStorage.stuLeaveBatch=$scope.stuAttendanceReport.batch;
					$http.get($localStorage.service+'StuAttendanceAPI/stuApproveLeave',{params:{courseId:$scope.stuAttendanceReport.course,batchId:$scope.stuAttendanceReport.batch},headers:{'access_token':$localStorage.access_token}})
					.success(function(data){
