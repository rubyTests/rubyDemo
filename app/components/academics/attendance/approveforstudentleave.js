angular
    ...
    .controller('approveforstudentleaveCtrl', [
        ...
        function ($scope,$rootScope,$timeout,$compile,variables,$resource,$filter,$stateParams,$http,$localStorage) {
			...
			if($stateParams.id){
				$http.get($localStorage.service+'StuAttendanceAPI/stuAttendanceReport',{params:{course:$localStorage.stuLeaveCourse,batchId:$localStorage.stuLeaveBatch,profileId:$stateParams.id,type:'approveLeave'},headers:{'access_token':$localStorage.access_token}})
				.success(function(data){
					...
					angular.forEach($scope.table_data,function(value,key){
						if($scope.table_data[key].approval=='Approved'){
							$scope.table_data[key].approval=true;
						}else{
							$scope.table_data[key].approval=false;
						}
					});
			...
			$scope.getApproveData=function(data){
				//alert("Get"+data.approval);
				//console.log(data,"data");
				if(data.approval==true){
					UIkit.modal.confirm('Are you sure? You want to Approve this leave', function(e) {
						$http({
						method:'POST',
						url: $localStorage.service+'StuAttendanceAPI/stuApproveLeave',
						data: {'ID':data.ID,'approval':data.approval},
					...
					UIkit.modal.confirm('Are you sure? You want to Reject this leave', function(e) {
						$http({
						method:'POST',
						url: $localStorage.service+'StuAttendanceAPI/stuApproveLeave',
						data: {'ID':data.ID,'approval':data.approval},
